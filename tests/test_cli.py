...
def test_cli_train_writes_loadable_checkpoint(tmp_path: Path) -> None:
    out = tmp_path / "m.npz"
    code = main([
        "train", "--steps", "2", "--batch", "4", "--seq-len", "16",
        "--data", "data/input.txt", "--out", str(out),
    ])
    assert code == 0
...
def test_cli_runs_as_module(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "transformer", "train", "--steps", "2",
         "--batch", "4", "--seq-len", "16", "--data", "data/input.txt", "--out", str(out)],
